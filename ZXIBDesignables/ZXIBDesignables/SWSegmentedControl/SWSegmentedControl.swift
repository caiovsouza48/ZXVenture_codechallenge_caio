...
open class SWSegmentedControl: UIControl {
    ...
    open func setTag(_ tag: Int, forSegmentAt index: Int) {
        guard let buttons = buttons, index < items.count else {
            return
        }
        let button = buttons[index]
    ...
    open func tagForSegmentAtIndex(_ segment: Int) -> Int{
        guard let buttons = buttons, segment < self.items.count else{
            return -1
        }
